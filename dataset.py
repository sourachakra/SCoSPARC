...
class CoData(data.Dataset):
    def __init__(self, img_root, gt_root, img_size, transform, max_num, is_train):
        ...
        # self.img_dirs2 = list(
        #     map(lambda x: os.path.join('/home/ubuntu/datasets/unlabeled_1_16/image/', x), class_list))
        self.gt_dirs = list(
            map(lambda x: os.path.join(gt_root, x), class_list))
        # self.gt_dirs2 = list(
        #     map(lambda x: os.path.join('/home/ubuntu/datasets/stego_dino_COCO9213_2/COCO9213/', x), class_list))
    ...
    def __getitem__(self, item):
        ...
        if self.is_train:
            names = os.listdir(self.img_dirs[item])
            num = len(names)
            img_paths = list(map(lambda x: os.path.join(self.img_dirs[item], x), names))

            gt_paths = list(map(lambda x: os.path.join(self.gt_dirs[item], x[:-4]+'.png'), names))

            im_list = os.listdir(self.img_dirs[item])

            names = []
            for i in range(len(im_list)):
                names.append(im_list[i])
        ...
        if self.is_train:
            ...
            return imgs, gts, subpaths, ori_sizes, cls_ls #, preds
        else:
            return imgs, gts, subpaths, ori_sizes #, preds #, rem_ims
...
if __name__ == '__main__':
    ...
    img_root = '/home/ubuntu/datasets/CoSOD3k/image/'
    gt_root = '/home/ubuntu/our_work/evaluation_metrics/root_dir/gt/CoSOD3k/'
