...
class IoU_loss(torch.nn.Module):
    ...
    def forward(self, pred, target):    
        b = pred.shape[0]
        #print('iou shapes',b,len(weights[0]))
        IoU = 0.0
        for i in range(0, b):
            #print('target:',target.size(),'pred:',pred.size())
            Iand1 = torch.sum(target[i, :, :, :]*pred[i, :, :, :])
            Ior1 = torch.sum(target[i, :, :, :]) + torch.sum(pred[i, :, :, :])-Iand1
            IoU1 = Iand1/(Ior1 + 1e-5)
            #if epoch > 6:
                #print('epoch > 10 weights:',weights)
            IoU = IoU + (1-IoU1)
...
# class Scale_IoU2(nn.Module):
#     def __init__(self):
#         super(Scale_IoU2, self).__init__()
#         self.iou = IoU_loss()

#     def forward(self, scaled_preds, scaled_gts):
#         loss = 0
#         for gt_lvl in scaled_gts[0:]:
#             for pred_lvl in scaled_preds[0:]:
#                 loss += self.iou(torch.sigmoid(pred_lvl), torch.sigmoid(gt_lvl)) + self.iou(1-torch.sigmoid(pred_lvl), 1-torch.sigmoid(gt_lvl))
#         return loss
