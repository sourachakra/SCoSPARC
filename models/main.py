...
class DPG(nn.Module):
    ...
    def forward(self, x5, num_l, num_u, gt_used,pts):
        # x: B,C,H,W
        print('x1:',x5.size())
        x5 = self.conv(x5)+x5
        B, C, H5, W5 = x5.size()
...
#         print('mult2 size:',(x5*proto2).size())
#         print('mask size:',mask.size())
        if gt_used:
            mask2 = mask[:,0,:,:].clone()
            for i in range(10):    
                img = normalize_im(mask2[i].cpu().numpy())
                #print('gg path:','../COCO9213/img_bilinear_224/'+pts[i][0])
                orig = cv2.resize(cv2.imread('../TransFGU-main/dino-main/unsupervised_cosal/COCO9213/img_bilinear_224/'+pts[i][0]),(224,224))
                im = cv2.resize(img,(224,224))
                im = cv2.applyColorMap(im, cv2.COLORMAP_JET)
                im = cv2.cvtColor(im,cv2.COLOR_BGR2RGB)
                im = cv2.addWeighted(im, 0.5, orig, 0.5, 0)
                ...
                if i == 0:
                    ...
                    img1 = np.concatenate((img1,im),1)

#             im_col = cv2.cvtColor(im,cv2.COLOR_GRAY2RGB)
#             joint = np.concatenate((im_col,org),0)
            cv2.imwrite('../'+str(i)+'.png',img1)
        
        if gt_used:
            return x5, proto2, x5*proto1+x51, mask, x51 #, proto_l, proto_u
        else:
            return x5, proto2, x5*proto1+x51, mask, x51
...
class DCFMNet(nn.Module):
    ...
    def forward(self, x, gt, num_l, num_u, gt_used,pts):
        if self.mode == 'train':
            preds = self._forward(x, gt, num_l, num_u, gt_used,pts)
        else:
            with torch.no_grad():
                preds = self._forward(x, gt, num_l, num_u, gt_used,pts)
    ...
    def _forward(self, x, gt, num_l, num_u, gt_used,pts):
        #print('x',x.size())
        [B, _, H, W] = x.size()
        x5, x4, x3, x2, x1 = self.featextract(x)

        feat, proto, weighted_x5, cormap,x51 = self.fusion(x5,num_l,num_u,gt_used,pts)

        feataug = self.aug(weighted_x5)
        print('feataug:',feataug.size())
...
class DCFM(nn.Module):
    ...
    def forward(self, x, gt, num_l, num_u, gt_used,pts):
        ########## Co-SOD ############
        preds = self.dcfmnet(x, gt, num_l, num_u, gt_used,pts)
        return preds
