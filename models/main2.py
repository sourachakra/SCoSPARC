...
class DSLayer(nn.Module):
    def __init__(self, in_channel=64):
        ...
        # self.predlayer2 = nn.Sequential(
        #     nn.Conv2d(64, 2, kernel_size=1, stride=1, padding=0))#, nn.Sigmoid())

    def forward(self, x):
        x = self.enlayer(x)
        #print('enc layer',x.size())
        #x1 = self.predlayer2(x)
        x = self.predlayer(x)
        #print('dec layer',x.size())
        return x #,x1
...
class DPG(nn.Module):
    ...
    def correlation(self, x5, seeds, num_l, num_u, gt_used):
        B, C, H5, W5 = x5.size()
        if self.training:
        ...
        #print('corr maps before size:', correlation_maps.size())
        # if gt_used:
        #     num_ims = correlation_maps.size()[1]
        #     h_num_ims = int(num_ims/2)
        #     w_l = (num_u)/(num_l+num_u) #(3/4)*
        #     w_u = (num_l)/(num_l+num_u) #(1/4)*
        #     w_l = w_l/num_l
        #     w_u = w_u/num_u
        #     cm_size = correlation_maps.size()
        #     correlation_maps_m = torch.zeros(cm_size[0],cm_size[2],cm_size[3]).cuda()
        #     for i in range(len(correlation_maps)):
        #         correlation_maps_m[i,:,:] = torch.mean(correlation_maps[i,0:num_l],0) + torch.mean(correlation_maps[i,num_l:],0)
        #     correlation_maps = correlation_maps_m
        #     correlation_maps = correlation_maps.view(B, -1)
        #     #correlation_maps = correlation_maps.mean(1).view(B, -1)

        # else:
        correlation_maps = correlation_maps.mean(1).view(B, -1)

        #print('corr maps after size:', correlation_maps.size())
    ...
    def forward(self, x5, num_l, num_u, gt_used):
        # x: B,C,H,W
        x5 = self.conv(x5)+x5
        B, C, H5, W5 = x5.size()
        ...
        #print('x_w before size:',x_w.size())
        # if gt_used:
        #     num_ims = x_w.size()[-1]
        #     #h_num_ims = num_l
        #     w_l = 1-(num_l)/(num_l+num_u) #(3/4)*
        #     w_u = 1-(num_u)/(num_l+num_u) #(1/4)*
        #     x_w_m = torch.zeros(len(x_w)).cuda()
        #     #for i in range(len(x_w)):
        #     x_w_m[:] = w_l*torch.sum(x_w[:,0:num_l],1) + w_u*torch.sum(x_w[:,num_l:],1)
        #     x_w = x_w_m
        #     #print(num_ims,h_num_ims)
        # else:
        x_w = x_w.mean(-1)
        
        #print('x_w after size:',x_w.size())
        ...
        cormap = self.correlation(norm0, seeds, num_l, num_u, gt_used)
        x51 = x5 * cormap
    ...
    def forward(self, weighted_x5, x4, x3, x2, x1, H, W):
        ...
        preds.append(
            F.interpolate(_pred,
                          size=(H, W),
                          mode='bilinear', align_corners=False))
        #binary_preds = F.interpolate(binary_preds,size=(H, W),mode='bilinear', align_corners=False)
...
class DCFMNet(nn.Module):
    ...
    def forward(self, x, gt, num_l, num_u, gt_used):
        if self.mode == 'train':
            preds = self._forward(x, gt, num_l, num_u, gt_used)
        else:
            with torch.no_grad():
                preds = self._forward(x, gt, num_l, num_u, gt_used)
    ...
    def _forward(self, x, gt, num_l, num_u, gt_used):
        [B, _, H, W] = x.size()
        x5, x4, x3, x2, x1 = self.featextract(x)
        feat, proto, weighted_x5, cormap = self.fusion(x5,num_l,num_u,gt_used)
        feataug = self.aug(weighted_x5)
        preds = self.decoder(feataug, x4, x3, x2, x1, H, W)
        if self.training and gt_used:
            gt = F.interpolate(gt, size=weighted_x5.size()[2:], mode='bilinear', align_corners=False)
            feat_pos, proto_pos, weighted_x5_pos, cormap_pos = self.fusion(x5 * gt,num_l,num_u,gt_used)
            feat_neg, proto_neg, weighted_x5_neg, cormap_neg = self.fusion(x5*(1-gt),num_l,num_u,gt_used)
...
class DCFM(nn.Module):
    ...
    def forward(self, x, gt, num_l, num_u, gt_used):
        ########## Co-SOD ############
        preds = self.dcfmnet(x, gt, num_l, num_u, gt_used)
        return preds
