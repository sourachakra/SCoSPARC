...
class DPG(nn.Module):
    ...
    def forward(self, x5, num_l, num_u, gt_used):
        ...
        x51 = x5 * cormap
#         print('x51 size:',x51.size())
        proto1 = torch.mean(x51, (0, 2, 3), True)
        proto2 = torch.mean(x51, (2, 3), True)
#         print('proto1 size:',proto1.size())
#         print('proto2 size:',proto2.size())
#         print('x5 size:',x5.size())
#         print('x51 size:',x51.size())
#         print('mult size:',(x5*proto1).size())
#         print('mask size:',mask.size())
        if gt_used:
            return x5, proto1, proto2, x5*proto1+x51, mask, x51 #, proto_l, proto_u
        else:
            return x5, proto1, x5*proto1+x51, mask, x51
...
class DCFMNet(nn.Module):
    ...
    def _forward(self, x, gt, num_l, num_u, gt_used):
        ...
        if self.training and gt_used:
            feat, proto, _, weighted_x5, cormap,x51 = self.fusion(x5,num_l,num_u,gt_used)
        else:
            feat, proto, weighted_x5, cormap,x51 = self.fusion(x5,num_l,num_u,gt_used)
        ...
        if self.training and gt_used:
            ...
            feat_pos, proto_pos, indiv_proto_pos, weighted_x5_pos, cormap_pos, x51_pos = self.fusion(x5 * gt,num_l,num_u,gt_used)
            #print('proto_pos size',proto_pos.size(),indiv_proto_pos.size())
            feat_neg, proto_neg, indiv_proto_neg, weighted_x5_neg, cormap_neg, x51_neg = self.fusion(x5*(1-gt),num_l,num_u,gt_used)
            
            with torch.no_grad():
                weights = torch.zeros(1,len(indiv_proto_pos)).cuda()
                for i in range(len(indiv_proto_pos)):
                    #print('sizes:',indiv_proto_pos[i].unsqueeze(0).size(),proto_pos.size())
                    weights[0,i] = Embed_loss(indiv_proto_pos[i].unsqueeze(0),proto_pos)
                #print('weights:',weights)
                #weights = weights/torch.sum(weights)
                #print('weights norm:',weights)
            return preds, proto, proto_pos, proto_neg #, weights
