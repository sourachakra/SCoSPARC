...
class SAMNet(nn.Module):
    ...
    def forward(self,x,paths,mode,idx,epoch,cut_off_epoch,dataset):
        ...
        preds_fin_noncrf = preds_fin.clone()#.unsqueeze(1)
        ...
        sel_th = 0.505+1.0*((1-avg_tot_conf)-0.48)  #Adaptive thresholding
        ...
        if epoch > -1:
            for i in range(len(preds_fin)):
                ...
                orig = cv2.imread('/home/schakraborty/cvpr_2023/fresh_try_unsup_cosal/datasets/'+dataset+'/image/'+paths[i][0][:-4]+'.jpg')
            ...
            try:
                ...
                print('CRF exception')
                preds_fin_round2_crf = preds_fin_round2 #.unsqueeze(1)
             

            if idx % 5 == 0 and mode == 'train':
                visualize_preds3(preds_fin_noncrf,caw,self_attn_maps2,'/home/schakraborty/cvpr_2023/fresh_try_unsup_cosal/datasets/COCO9213/img_bilinear_224/','/home/schakraborty/cvpr_2023/fresh_try_unsup_cosal/datasets/COCO9213/gt_bilinear_224/',paths,epoch,idx)
